function varargout = KalmanF03(varargin)
% KALMANF03 MATLAB code for KalmanF03.fig
%      KALMANF03, by itself, creates a new KALMANF03 or raises the existing
%      singleton*.
%
%      H = KALMANF03 returns the handle to a new KALMANF03 or the handle to
%      the existing singleton*.
%
%      KALMANF03('CALLBACK', hObject, eventData, handles, ...) calls the local
%      function named CALLBACK in KALMANF03.M with the given input arguments.
%
%      KALMANF03('Property', 'Value', ...) creates a new KALMANF03 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KalmanF03_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KalmanF03_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KalmanF03

% Last Modified by GUIDE v2.5 16-Oct-2013 01:51:28

% Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name', mfilename, ...
                       'gui_Singleton', gui_Singleton, ...
                       'gui_OpeningFcn', @KalmanF03_OpeningFcn, ...
                       'gui_OutputFcn', @KalmanF03_OutputFcn, ...
                       'gui_LayoutFcn', [], ...
                       'gui_Callback', []);
...
% --- Executes just before KalmanF03 is made visible.
function KalmanF03_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KalmanF03 (see VARARGIN)
    handles = initialize_gui(hObject, handles);
    % Choose default command line output for KalmanF03
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

% UIWAIT makes KalmanF03 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = KalmanF03_OutputFcn(hObject, eventdata, handles) 
...
% hObject    handle to Angular_velocity (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject, 'String') returns contents of Angular_velocity as text
%        str2double(get(hObject, 'String')) returns contents of Angular_velocity as a double
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    [mu0, X0, theta, r, q, Sigma0, vl, va, freq, m, b, iter] = create_var(hObject, handles);
    Esercitazione3(mu0, X0, theta, r, q, Sigma0, vl, va, freq, m, b, iter, hObject, handles);
...
function set_Default(hObject, handles)
    ...
    set(handles.Angular_velocity, 'String', 0.1);
...
function [mu0, X0, theta, r, q, Sigma0, vl, va, freq, m, b, iter] = create_var(hObject, handles)
    ...
    Angular_velocity = eval(get(handles.Angular_velocity, 'String'));
...
function Esercitazione3(mu0, X0, theta, r, q, Sigma0, vl, va, freq, m, b, iter, hObject, handles)
%La Funzione implementa il Filtro di Kalman esteso in presenza del muro e
%angolo di sterzata
    ...
    U = [vl; vl; va]; %Vettore di controllo sul robot
    I = eye(3); %Matrice identita' utilizzata nel filtro di Kalman esteso

    A = [1 0 0; 0 1 0; 0 0 1];
    B = [cos(theta)*dt 0 0; 0 sin(theta)*dt 0; 0 0 dt];
    C = [-2*m/(sqrt(m*m+1)), 2/(sqrt(m*m+1)) 0];

    R = r*eye(3); %matrice di covarianza del processo
    ...
    for i = 1 : iter
        ...
        G = [1, 0, -U(1, 1)*sin(theta)*dt; 0 1 U(2, 1)*cos(theta)*dt; 0 0 1]; %Jacobiano

        %Fase di Predizione
        mu_pred(1, 1) = muk(1, 1) + U(1, 1)*cos(muk(3, 1))*dt;
        mu_pred(2, 1) = muk(2, 1) + U(2, 1)*sin(muk(3, 1))*dt;
        mu_pred(3, 1) = muk(3, 1) + U(3, 1)*dt;

        Sigma_pred = G*(Sigmak*G') + R;
        ...
        %Memorizzazione dati
        matrix_xk(i, 1) = Xk(1, 1);
        matrix_xk(i, 2) = Xk(2, 1);
        matrix_mu_pred(i, 1) = mu_pred(1, 1);
        matrix_mu_pred(i, 2) = mu_pred(2, 1);
        matrix_muk(i, 1) = muk(1, 1);
        matrix_muk(i, 2) = muk(2, 1);
        matrix_err(i, :) = errore;
        matrix_sigmak(:, :, i) = Sigmak(1:2, 1:2);
